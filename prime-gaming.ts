...
const screenshot = (...a: string[]) => resolve(cfg.dir.screenshots, 'prime-gaming', ...a);
...
try {
  ...
  while (await page.locator('button:has-text("Sign in")').count() > 0) {
    ...
    if (email && password) {
      ...
      page.waitForURL('**/ap/mfa**').then(async () => {
        ...
        const otp = cfg.pg_otpkey && authenticator.generate(cfg.pg_otpkey) || await prompt({ type: 'text', message: 'Enter two-factor sign in code', validate: (n: string) => n.toString().length == 6 || 'The code must be 6 digits!' }); // can't use type: 'number' since it strips away leading zeros and codes sometimes have them
  ...
  const waitUntilStable = async (f: () => Promise<unknown>, act: () => Promise<void>) => {
    let v: unknown;
    while (true) {
      const v2 = await f();
  ...
  const scrollUntilStable = async (f: () => Promise<unknown>) => await waitUntilStable(f, async () => {
  ...
  internal.reverse();
  external.reverse();
  const sameOrNewPage = async (url: string): Promise<{ p: Page, isNew: boolean }> => {
  ...
  const skipBasedOnTime = async (url: string): Promise<boolean> => {
    ...
    if (await page.locator('div:has-text("Link game account")').count() // TODO still needed? epic games store just has 'Link account' as the button text now.
      ...
      if (store in redeem) { // did not work for linked origin: && !await page.locator('div:has-text("Successfully Claimed")').count()
        ...
        if (store == 'legacy games') { // may be different URL like https://legacygames.com/primeday/puzzleoftheyear/
          redeem[store] = await page.locator('li:has-text("Click here") a').getAttribute('href') as string; // full text: Click here to enter your redemption code.
        }
  ...
  if (cfg.pg_claimdlc) {
    ...
    const dlc_unlinked: Record<string, string[]> = {};
  ...
  if ((error as Error).message && process.exitCode != 130) notify(`prime-gaming failed: ${(error as Error).message.split('\n')[0]}`);
} finally {
